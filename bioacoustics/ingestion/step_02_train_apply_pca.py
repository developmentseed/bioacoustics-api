...
def main():
    ...
    embeddings_blobs = [
        ...
        for b in utils.storage_client.list_blobs(
            utils.BUCKET_NAME, prefix=f"vector_{utils.EMBEDDINGS_FOLDER}"
        )
    ]
        ...
        with tempfile.NamedTemporaryFile(prefix="/data/") as tmpfile:
            ...
            try:
                ...
                rand_indexes = np.random.randint(
                    low=0,
                    high=len(embeddings),
                    size=int(PCA_TRAINING_SAMPLE_SIZE * len(embeddings)),
                )
    ...
    with tempfile.NamedTemporaryFile(prefix="/data/") as tmpfile:
        ...
        blob = utils.bucket.blob(
            f"admin_{utils.EMBEDDINGS_FOLDER}/1280_to_256_dimensionality_reduction.pca"
        )
        ...
        with tempfile.NamedTemporaryFile(prefix="/data/") as tmpfile:
            ...
            try:
                ...
                with tempfile.NamedTemporaryFile(prefix="/data/") as tmpfile:
                    ...
                    blob = utils.bucket.blob(
                        f"reduced_vector_{utils.EMBEDDINGS_FOLDER}/{reduced_vector_blob_name}"
                    )
                    blob.upload_from_filename(tmpfile.name)
            except Exception:
                logger.exception(f"Failed to load {embeddings_blob.name}")
