...
from .audio_utils import log_scale, pcen
from .signal import linear_to_mel_weight_matrix
from .cwt import melspec_params, gabor_filter, Domain, Normalization, convolve_filter
...
class MorletWaveletTransform(Frontend):
  ...
  def __call__(self, inputs: jnp.ndarray, train: bool = True) -> jnp.ndarray:
    ...
    gabor_filter = gabor_filter(
        sigma, Domain.TIME, ç.L1
    )
    filtered_signal = convolve_filter(
        gabor_filter,
        input_signal,
        gabor_std,
        Normalization.L1,
